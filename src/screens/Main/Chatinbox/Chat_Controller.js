...
import Chat from '../Chatinbox/Chat'
...
import AsyncStorage from '@react-native-community/async-storage';
  ...
  const Chat_Controller =(props)=> {
    ...
    const firebaseGet = async()=>{
       ...
       let value1 = await AsyncStorage.getItem('user');
        ...
        let postvalue =  await AsyncStorage.getItem('selectinbox')
  ...
  console.log('Your chat Item is here................',postvalueparsed1)
     ...
     setProgress(true)
      ...
      //  setRyderId(user._id)
      // let id = orderObj._id
      db().ref(`/messages/${postvalueparsed1.post_id+"-"+postvalueparsed1.receiver_id+"-"+postvalueparsed1.sender_id}`)
      .on('value', snapshot => {
...
const handleIDs =async (item) => {
  ...
  let value1 = await AsyncStorage.getItem('user');
            ...
            setuserid(parsed.id)
             ...
             setpostid(parsed1.post_id)
              setposttitle(parsed1.post_title)
...
const sendMessages = async()=>{
  ...
  console.log("receiver_id::::::",postvalueparsed1.receiver_id) 
  console.log("sender_id::::::",postvalueparsed1.sender_id) 
  console.log("post_id:::::",postvalueparsed1.post_id) 
...
let msg = {
  ...
  createdAt:Date(),
  sender_id:user.id,
  post_id:postvalueparsed1.post_id,
  last_message:text,
  }
 let newr;
 //post id + receiver + sender
 newr = db().ref('/messages').child(postvalueparsed1.post_id+"-"+postvalueparsed1.receiver_id+"-"+postvalueparsed1.sender_id).push(
  msg
 );
 setProgress(false)
 console.log("msg sucessfully push::::")
 ...
 //Api calling for chat 
  ...
  fetch('https://thaikadar.com/api/save-inbox',
  {
     ...
     body: JSON.stringify({
      ...
      sender_id:postvalueparsed1.sender_id,
      receiver_id:postvalueparsed1.receiver_id,
      inbox_id:0,
      post_id: postvalueparsed1.post_id, 
      last_message:text,
